...
/* ============================================================ component */
const vortexContainer =
    document.getElementById(
        "alice-vortex-container"
    );
...
if (
    ...
    const configRef = {
        current: {
            floorRadius:
                DEFAULTS.bottomRadius
                / PX_PER_WORLD,

            waistRadius:
                DEFAULTS.waistRadius
                / PX_PER_WORLD,
            ...
            lineCount:
                DEFAULTS.lineOptions.count,

            lineColor:
                DEFAULTS.lineOptions.color,

            lineGlow:
                DEFAULTS.lineOptions.glow
                / 10
                * LINE_GLOW_MAX,
            ...
            dotCount:
                DEFAULTS.dotOptions.count,

            dotSize:
                DEFAULTS.dotOptions.size
                / DOT_SIZE_SCALE,

            dotColor:
                DEFAULTS.dotOptions.color,

            dotGlow:
                DEFAULTS.dotOptions.glow
                / 10
                * DOT_GLOW_MAX,

            dotFlicker:
                DEFAULTS.dotOptions.flicker
                / 10,
            ...
            cometCount:
                DEFAULTS.cometOptions.count,

            cometSpeed:
                DEFAULTS.cometOptions.speed
                / 10
                * COMET_SPEED_MAX,

            cometColor:
                DEFAULTS.cometOptions.color,

            cometGlow:
                DEFAULTS.cometOptions.glow
                / 10
                * COMET_GLOW_MAX,

            cometTail:
                DEFAULTS.cometOptions.tail,

            cometDelay:
                DEFAULTS.cometOptions.delay,

            collideForce:
                DEFAULTS.cometOptions.collide
                / 10,
            ...
            repelRadius:
                DEFAULTS.repelOptions.radius,

            repelStrength:
                DEFAULTS.repelOptions.strength,
    ...
    try {
        ...
        console.log(
            "Alice vortex initialized."
        );

    } catch (error) {
        console.error(
            "Could not initialize Alice vortex:",
            error
        );
    }
} else {
    console.error(
        "Alice vortex container or canvas "
        + "was not found."
    );
}
